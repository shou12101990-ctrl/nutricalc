...
class _NotePageState extends State<NotePage>
  ...
  static final _sections = <_NoteSection>[
    ...
    _NoteSection(
      ...
      title: 'つかいかた',
      color: Color(0xFF4A90D9),
      body: '重症患者や特殊病態のある患者の栄養処方を「個別計算・逆引き・自動設計」の3方式で支援し, トレンドとリスクを可視化する. \n\n①TOP画面（ベッド管理）\n・右上の「新規入室」から患者を登録(患者ID・ベッド・年齢・身長・体重・性別). カードをタップで再編集, 「計算」で計算画面へ, 「退室」で退室. \n・エネルギー式(簡易式kcal/kg〔既定〕/Harris-Benedict/Mifflin/間接熱量測定), 目標タンパク(g/kg), 絶食開始日, 病態タグを設定. 肥満(BMI≥30)は補正体重で自動補正. \n\n②計算画面（上中下の3エリア）\n(i) 患者カード(上)… 患者情報・目標を表示. 病態タグを付けると, 病態別の栄養設計の注意点・推奨が見られる. \n(ii) 個別選択 / ゼロmenu / 自動設計(中・3タブ)\n・「個別選択」… 各製剤の使用量に応じて栄養素を合算するモード(EN/TPN/PPN/食事/加注を横断集計). \n・「ゼロmenu」… 合計の熱量とタンパク・脂質投与量を設定したとき, 各成分製剤の使用量を逆引き計算するモード(ゼロから組むメニューなので便宜的にゼロmenuとした). \n・「自動設計」… 絶食〜経口リハ開始までのイベント時期を設定すると, 毎日の処方をフェーズに応じて自動設計し提案するモード(急性期20→EN導入25→回復30 kcal/kg, Refeedingは初期kcalを自動cap). \n(iii) サマリー / トレンド(下)\n・「サマリー」… 合計IN・総カロリー・タンパク投与量, 各製剤の必要処方数と流量指示を, カルテ記載/処方向けに表示. \n・「トレンド」… 日々のIN・タンパク投与量・カロリーやその内訳の変化を可視化. \n\n■ 製剤選択アルゴリズム（病態連動）\n・病態を選ぶと, 目標タンパクの推奨範囲, ゼロmenuのNPC/N比・脂質量を自動調整. \n・GIR(糖)>5(糖質制限病態は>4), 脂質>1.0–1.5 g/kg/dayでアラート. 自動設計は上限を超えない製剤選定. \n・微量栄養素は全ソース合算し耐容上限超過を警告. 胆汁うっ滞/肝障害はMn-free製剤へ切替提案, CRRTはSe/B1補充, 高排出消化管はZn補充, 糖負荷前のB1未投与はWernicke警告. \n・加注はベース製剤の内蔵成分と重複しないよう自動付替え(オールインワンTPNには加注しない 等). \n\n■ アイコン・色の意味\n・入室(緑・login) / 絶食開始(赤・no_meals) / 栄養開始(青・water_drop) / 経口リハ導入(食事・restaurant). \n・日付の丸囲み＝入室からday5の倍数. \n・グラフ棒: 下から PN→EN→食事 の積み上げ. 青の折れ線＝IN(水分量), IN最高点に水平線＋_ml表記. 赤線＝アミノ酸(AA). \n・グラフ下の「リスクと補充サジェスト」をタップで各種アラートを展開. \n\n──────────\n■ 計算アルゴリズム / 仕様\n\n◇ エネルギー目標(energy.dart)\n4式から選択(患者編集). 既定はkcal/kg式. \n・H-B(Long): 男 66+13.7W+5.0H−6.8A / 女 655+9.6W+1.8H−4.7A. ×AF×SF. \n・Mifflin-St Jeor: 男 10W+6.25H−5A+5 / 女 同−161. ×AF×SF. \n・kcal/kg(ASPEN/JSPEN): feedingWeight×25〜30. AF/SFは適用しない. \n・間接熱量測定: 測定REE×AF. SFは適用しない. \n\n◇ 栄養計算用体重 feedingWeight(body_weight.dart, 内部自動)\n・実体重<IBW(るい痩) → 実体重\n・BMI<30 かつ 実体重>1.20×IBW → 補正体重 = IBW+0.25×(実−IBW)\n・BMI 30–50 → 実体重(kcal/kg式は別途11–14でcap)\n・BMI>50 → IBW\n・タンパクはBMI≥30でIBWを使用(ASPEN: 30–40→2.0, ≥40→2.5 g/kg IBW). \n\n◇ IBW(理想体重)\n・H≥152cm: Devine 男50/女45.5 +0.9×(H−152)\n・H<152cm: BMI-22換算 22×(H/100)² (Devineが破綻するため)\n\n◇ タンパク目標(protein.dart, 病態連動)\n・通常 1.0〜1.5 / 重症 1.2〜2.0 (ASPEN, ESPEN既定1.3を漸増)\n・腎(保存期) 0.6–0.8 / AKI(非重症) 0.8–1.0 / 重症+RRTなし 1.0→1.3漸増\n・間欠RRT 1.3–1.5 / CRRT/PIRRT 1.5–1.7 / 肝硬変 1.0–1.2\n・熱傷>20%TBSAはENグルタミン 0.3–0.5 g/kg/日を10–15日追加\n・範囲はサジェストのみで強制クランプしない. \n\n◇ ゼロmenu 病態係数 自動セット(conditions.dart)\n病態タグでNPC/N比中央値・脂質g/kg中央値・糖質制限を自動設定. \n・一般 175 / 0.9 / 否     ・重症 100 / 0.9 / 要(GIR≤4)\n・腎(保存期) 350 / 0.9 / 否    ・腎(透析) 150 / 0.9 / 軽\n・肝 150 / 1.0 / 否        ・呼吸 175 / 1.0 / 要(脂質↑/糖↓)\n・耐糖能異常 175 / 1.0 / 要(血糖140–180)\n複数病態の解決: NPC/N=最大, 脂質=最小(上限1.0), 糖質制限=OR. \n\n◇ GIR / 脂質速度 hard limit (infusion.dart)\n・GIR 警告 ≥4.0 / 上限 5.0 mg/kg/min(糖質制限病態は警告4). 分子は静脈ブドウ糖のみ(EN/食事/glycerolは除外). \n・脂質速度 警告 0.10 / 上限 0.125 g/kg/h. \n・脂質1日量 警告 1.0 / 上限 1.5 g/kg/day. \n・個別/ゼロmenu: 超過時アラート(医師判断に委ねる). \n・自動設計: 超えない製剤選定. 余剰kcalは脂質→IN増で吸収. \n\n◇ Refeeding tier(refeeding.dart, NICE CG32準拠)\n・EXTREME: BMI<14 / 絶食>15日\n・HIGH(1基準): BMI<16 / 体重減>15%(3–6mo) / 絶食>10日 / K・P・Mg低値\n  または(2基準): BMI<18.5 / 体重減>10% / 絶食>5日 / アルコール・薬物歴\n自動設計の初期kcal ramp: \n・EXTREME: 5→10→15→20→full kcal/kg\n・HIGH: 10→15→20→full\nB1はrefeedingリスク+糖質投与で200 mg/dayを初期10日 自動加注. \nRSイベント検出(refeeding_events.dart): P/K/Mg前日比 ≥10%(mild) / ≥20%(moderate) / ≥30% or 臓器障害(severe). \n\n◇ EN開始/遅延 判断(en_timing.dart, ESPEN ICU 2019)\n・回避基準(7): 制御不能ショック / 高度低灌流 / 上部消化管出血 / 腸管虚血 / 高排出フィステル / 腹部コンパートメント / GRV>500\n・早期支援基準(12): ECMO / TBI / 脳卒中 / 脊損 / 重症膵炎 / 消化管術後 / AAA術後 / 腹部外傷(連続性保持) / 筋弛緩 / prone / 開腹 / 下痢\n回避が1つでも→遅延. 早期+回避なし→早期開始. 双方なし→標準. \n\n◇ 採血提案(lab_schedule.dart)\n・Refeedingリスク: P/K/Mgを1日1回(超高リスクは1日1–2回)を5日, 以降2–3日毎. \n・CKRT稼働: Se/Zn/Cu/水溶性ビタミン+CRP/Alb 週1, Cu評価は14日以降. \n・長期PN(≥7日): 電解質/微量元素/肝胆道系(AST/ALT/T-Bil/ALP) 週1(IFALD監視). \n・常時: 電解質/腎機能/血糖を数日毎. \n\n◇ 微量元素・ビタミン UL監視(micronutrients.dart, 食事摂取基準2025)\n全ソース(EN/PN/食事/加注)を日毎に合算しUL/必要量と比較. \n・Na: 食塩>7 g/日で黄, ≥9.6 g/日で赤(=日本人平均摂取量). \n・K: 100 mEq/日超で速度(≤20 mEq/h)・濃度(≤40 mEq/L)・心電図監視警告. \n・Mn: 胆汁うっ滞/肝障害はMn-free製剤(ボルビサール)への切替を提案. \n・Zn: PN<3 mg かつ 高排出消化管/創傷で補充提案. \n・Se: PN<60 µg かつ CRRT/長期/熱傷で補充提案. \n\n◇ Repair Loop(repair.dart)\nアラートに対し候補処方を生成しスコアでランキング. \n・GIR過剰 → 低濃度品へswap / 流量減 / 脂質追加でkcal維持\n・B1欠落 → 高用量B1製剤を糖質投与前に挿入\n・Mn過剰 → Mn-free 微量元素複合へswap\n・Zn/Se不足 → 補充製剤を追加\n採用は医師判断(自動適用はしない). \n\n◇ 加注(混注)の自動付替え\nベース製剤の内蔵成分(MV/微量元素/電解質)を検出し, 重複しないよう加注を選択. \n・オールインワンTPN(エルネオパNF/ワンパル) → 加注なし(Seのみ条件付き)\n・フルカリック(MVのみ内蔵) → 微量元素のみ追加\n・ミキシッド/ゼロmenu → MV + 微量元素\n\n◇ 個別選択タブの集計(builder_page.dart)\nEN/TPN/PPN/食事/加注の全ソースを横断集計. 微量栄養素は流量指定時の按分係数(マクロと同一)で合算しdouble countを防ぐ. \n\n◇ 臨床イベントオーバーレイ(clinical_event/event_overlay.dart)\n自動設計の固定テンプレートは保持したまま, 経過中の臨床イベントを各Dayに「重ねて」derived plan(イベント反映後の処方)を生成する. テンプレ(基本フェーズ)とderived(イベント適用後)は別管理. \n・追加: 自動設計の設定内「臨床イベント」→追加. 種別・開始/終了Day・重症度・(RRTは)モダリティとパラメータを設定. 終了Day空欄=open-ended(タイムライン終端まで継続). \n・種別と効果: EN hold(速度固定) / EN不耐(減量) / 反復NPO(禁食) / NOMI疑い(経腸ハード遮断→PNのみ) / refeeding低P(48hエネルギー制限) / BUN上昇(タンパク見直し) / 胆汁うっ滞(Mn-free) / 溢水(水分cap・高濃度) / RRT開始(IRRT/SLED/CRRT) / RRT停止(次モダリティへ遷移). \n・次元別解決: 経路/エネルギー/タンパク/水分/電解質/微量栄養素/製剤フィルタを独立に解決し, 別イベントの効果が共存(例: CRRTのタンパクバンドとrefeeding低Pの48h制限は同時適用). \n・優先度(制限的な順): NOMI > refeeding低P・NPO > RRT > 溢水 > 肝/胆汁うっ滞 > EN不耐 > EN hold > BUN. 最も制限的な経路が支配. \n・RRTタンパク: IRRT 1.3–1.5 / SLED 1.5–1.7(PIKRT様) / CRRT 1.5–1.7 g/kg/日. 稼働中は安定CKDの保存的制限を抑制. CRRT≥14日でCu(銅)評価ノート. SLED/CRRTでKRT関連液(クエン酸/乳酸/ブドウ糖)カロリー未入力は情報注記. \n・出典タグ(source_tier): 各ルールにガイドライン(必須/バンド/義務)/ローカル方針/UXプリセット/ユーザーイベント等のティアを付与し, 由来を区別(固定ENラダー等は臨床強制でないUXプリセット). ',
    ),
    _NoteSection(
      category: 'EN',
      title: '経腸栄養 (EN)',
    ...
    _NoteSection(
      ...
      body: '■ 健常成人の基準（厚労省 食事摂取基準2025）\n'
          '・推奨量(RDA): 男性 65 g/日, 女性 50 g/日. 目標量(DG): 13〜20%エネルギー(50〜64歳14〜20, 65歳以上15〜20%). \n'
          '・重症・侵襲下はこの健常基準を大きく上回る需要になる(下記ICU目標). \n\n'
          '■ 考え方（侵襲時）\n'
          '・全エネルギーの10〜20%をタンパクで補うのが基本. \n'
          ...
          '■ ICUの投与量目標（ASPEN/ESPEN/JSPEN）\n'
          '・ASPEN/SCCM 2016: 1.2〜2.0 g/kg(実体重)/日. \n'
          '・ESPEN 2019: 1.3 g/kg/日を急性期は段階的に漸増. \n'
          '・JSPEN: 重症は1.2〜2.0を目安. 非重症は最低1.0, 目標1.2〜1.5 g/kg/日. \n'
          '・肥満(ASPEN): BMI30〜40で2.0, ≥40で2.5 g/kg(理想体重). \n'
          '・肝硬変: 1.0〜1.2 g/kg/日(肝性脳症でも過度なタンパク制限はしない). \n\n'
          '■ NPC/N比（非タンパクカロリー/窒素）\n'
          '・「タンパクが効率よく利用されるために必要な非タンパクカロリー量」の指標(窒素1 g = タンパク質6.25 g). \n'
    ...
    _NoteSection(
      ...
      body: '■ 健常成人の基準（厚労省 食事摂取基準2025）\n'
          '・目標量(DG): 炭水化物 50〜65%エネルギー. 食物繊維 男性21 g/女性18 g以上/日. 添加糖はできるだけ少なく. \n\n'
          '■ 考え方（栄養療法）\n'
          '・全エネルギーの50〜65%を炭水化物で補う(従来60〜70%とされたが, 過剰糖負荷を避け抑制傾向). \n'
          '・末梢から投与できる加糖液の濃度限界は10%. \n\n'
          '■ 投与量上限 (hard limit)\n'
          'ESPEN/JSPENはPNのグルコースまたはENの炭水化物について, ICU患者では5 mg/kg/minを超えないよう推奨している. \n'
    ...
    _NoteSection(
      ...
      body: '■ 健常成人の基準（厚労省 食事摂取基準2025）\n'
          '・目標量(DG): 脂質 20〜30%エネルギー. 飽和脂肪酸 7%エネルギー以下. n-6系/n-3系脂肪酸に目安量あり. \n\n'
          '■ 考え方（栄養療法）\n'
          '・全エネルギーの20〜30%を脂質で補う(過剰な糖負荷を避けNPC/N比・糖質量を適正化する目的でも用いる). \n'
